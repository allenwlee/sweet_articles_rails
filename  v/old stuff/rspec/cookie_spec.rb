...
describe Cookie do
  ...
  describe "#bake!" do
    ...
    it "returns the cookie object" do
      cookie.bake!(10).should be_kind_of(Cookie)
    end
  ...
  describe "#status" do
    it "should be one of blah... --Keith Tom" do
      [:doughy, :almost_ready, :ready, :burned].should include(cookie.status)
    end
    ...
    context "when baked for at least 7 but less than 10 minutes" do
      it "is `:almost_ready`" do
        cookie.bake!(7).status.should eq :almost_ready
      end
    end
    ...
    context "when baked for at least 12 minutes" do
      it "is `:burned`" do
        cookie.bake!(2000).status.should eq :burned
      end
    end
  end
end
